<template>
  <div class="review-container">
    <el-card>
      ...
      <!-- Search (Only show for 'all' tab, or customize for warning) -->
      <el-form :inline="true" :model="queryParams" class="search-form mt-20" v-if="activeTab === 'all'">
...
const getList = async () => {
  loading.value = true
  try {
    const url = activeTab.value === 'warning' ? '/review/warning' : '/review/page'
    const res = await request.get(url, { params: queryParams })
...
const handleTabChange = () => {
  queryParams.page = 1
  // Reset filters if switching to warning, maybe? Or keep them?
  // Warning API doesn't take params other than page/size in doc, but usually it might.
  // I'll stick to basic params for warning as per doc.
  getList()
}
