<template>
  <div class="order-container">
    ...
    <el-dialog v-model="abnormalVisible" title="异常订单（超时未接）" width="800px">
       ...
       <!-- Simple pagination for abnormal if needed, for now just show list -->
    </el-dialog>
  </div>
</template>
...
const getList = async () => {
  loading.value = true
  try {
    const res = await request.get('/order/page', { params: queryParams })
...
const showAbnormal = async () => {
  const res = await request.get('/order/abnormal', { params: { page: 1, pageSize: 100 } })
